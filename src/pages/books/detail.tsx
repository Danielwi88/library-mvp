...
export default function BookDetail() {
  ...
  const reviewMutation = useMutation({
    ...
    onMutate: async (v) => {
      ...
      const previous = qc.getQueryData<{ book: import('@/services/books').BookDetail; reviews: Review[] }>(["book-detail", id]);
      ...
      if (previous) {
        qc.setQueryData(["book-detail", id], { ...previous, reviews: [me, ...previous.reviews] });
      }
      return { previous };
    },
    onError: (_err, _v, ctx) => {
  ...
  const relatedQ = useQuery({ queryKey: ["related", id], queryFn: () => fetchBooks({ page: 1, limit: 6 }) });
    ...
    <div className="space-y-8">
      ...
      <div className="rounded-xl border bg-white p-4 sm:p-5">
        <div className="grid gap-6 md:grid-cols-[280px,1fr] items-start">
          {/* Left: cover + metadata */}
          <div>
            <CoverImage src={b.coverUrl} alt={b.title} index={0} className="rounded-xl w-[321px] h-[482px] object-cover border" />
            <div className="mt-2 text-xs inline-flex items-center gap-1 text-neutral-700">
              <img src="/star.svg" className="w-3.5 h-3.5" /> {b.rating.toFixed(1)}
            </div>
            <div className="mt-2 text-xs font-semibold text-neutral-800">{b.author.name}</div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <h1 className="text-[22px] sm:text-[24px] leading-[30px] font-extrabold text-neutral-900">{b.title}</h1>
            ...
            <div className="grid grid-cols-3 divide-x rounded-xl border">
              <div className="p-4 text-center">
                <div className="text-[18px] font-extrabold">{b.totalCopies ?? 0}</div>
                <div className="text-[11px] text-neutral-600 mt-1">Page</div>
              </div>
              <div className="p-4 text-center">
                <div className="text-[18px] font-extrabold">{b.rating.toFixed(0)}</div>
                <div className="text-[11px] text-neutral-600 mt-1">Rating</div>
              </div>
              <div className="p-4 text-center">
                <div className="text-[18px] font-extrabold">{b.reviewCount ?? 0}</div>
            ...
            <div>
              ...
              <p className="text-[12px] text-neutral-700 leading-6">{b.description}</p>
            </div>
            <div className="flex items-center gap-2 pt-1">
              <Button variant="outline" onClick={() => dispatch(add({ bookId: b.id, title: b.title, coverUrl: b.coverUrl }))} className="rounded-full">Add to Cart</Button>
              <Link to="/checkout"><Button className="rounded-full">Borrow Book</Button></Link>
              <button aria-label="Share" className="ml-1 size-9 grid place-items-center rounded-full border text-neutral-700">↗</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-2 text-[12px] text-neutral-700">
          ...
          <span className="text-neutral-600">({b.reviewCount ?? reviews.length} Ulasan)</span>
        </div>
          ...
          {reviews.map(r => (
            <div key={r.id} className="rounded-xl border p-4 bg-white">
              <div className="flex items-center gap-2">
                <img src="/avatarfall.svg" className="w-8 h-8 rounded-full" />
                <div>
                  <div className="text-sm font-medium">{r.user.name}</div>
                  <div className="text-xs text-muted-foreground">{dayjs(r.createdAt).format('DD MMM YYYY, HH:mm')}</div>
                </div>
              </div>
              <div className="mt-2">
                <div className="flex items-center gap-1">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <img key={i} src="/star.svg" className={"w-3.5 h-3.5" + (i < Math.round(r.rating) ? "" : " opacity-30")} />
                  ))}
                </div>
              </div>
        ...
        <form className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2" onSubmit={handleSubmit((v) => { reviewMutation.mutate(v); reset({ rating: 5, comment: "" }); })}>
          <Input type="number" min={1} max={5} step={1} className="w-28" {...register("rating", { valueAsNumber: true })} />
