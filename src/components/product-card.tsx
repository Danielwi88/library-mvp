...
export function ProductCard({ id, title, authorName, authorId, coverUrl, rating = 0, index = 0, compact = false, showDetailButton = false, className = "" }: Props) {
  ...
  return (
    <Link to={`/books/${id}`} className={`rounded-t-xl overflow-hidden border-none shadow-[0_0_20px_0_rgba(203,202,202,0.25)]  bg-white dark:bg-background block ${className}`}>
      <Card className="rounded-2xl overflow-hidden">
        <CardContent className="p-0">
          <img src={src} onError={onError} alt={title} className="w-full h-[336px] object-cover" />
          <div className={infoPad}>
            <div className="text-sm sm:text-lg font-bold truncate text-neutral-900 dark:text-foreground" title={title}>{title}</div>
            <div className="text-sm sm:text-md font-medium dark:text-muted-foreground text-neutral-700 truncate">
              {authorId !== undefined ? (
                <button
                  type="button"
                  onClick={(e) => {
            ...
            <div className="flex items-center justify-between mt-2">
              <span className="inline-flex items-center gap-1 text-sm sm:text-md ">
                <img src="/star.svg" alt="rating" className="size-6 text-sm sm:text-md font-semibold text-neutral-900" />
                {rating.toFixed(1)}
              </span>
              {showDetailButton && (
